    ...
    for (chr_row, chr_col) in combinations(chr_seq.keys(),r=2):
        ...
        def simulator_1_bloc(n_row, n_col, c_i, c_j, sig_2, intensity, noisy=noisy):
            ...
            if noisy:
                ...
                # sig = np.sqrt(sig_2)
                # noise[int(c_i)-int(sig) : int(c_i)+int(sig)+1, :] = 0
                # noise[:, int(c_j)-int(sig) : int(c_j)+int(sig)+1] = 0
...
def get_simulations_sbi_pretrained_transf(proposal, round, nb_train, DNN):
...
if 1:
    ...
    # path_dnn = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/transformer/sin_cos_4_levels_pos/mlp/"
    path_dnn = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/summary_stat/CNN/sigmoid/sequential/"
    
    path_sbi = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/sbi/"
    nb_seq = 10
    patch_size = 4
    DNN = torch.load(path_dnn+"dnn.pkl", map_location=torch.device('cpu'), weights_only=False)
    ...
    for k in range(nb_seq+1):
        ...
        # theta, theta_reg = get_simulations_sbi_pretrained_transf(proposal, k, nb_train, DNN)
        ...
        # proposal = posterior.set_default_x(DNN(C_ref))
...
if 0:
    ...
    nb_seq = 10
    path_sbi = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/sbi/pretrained/nsf/transformer/"
