...
def loss(theta, theta_hat):
    ...
    return torch.mean((theta_hat-theta)**2)
...
if 1:
    ...
    while epoch <= 100:
        ...
        for chr_id in range(1,17):
            ...
            for batch in train_loader['chr'+str(chr_id)]:
                ...
                features = cnn_subnet(x_batch.unsqueeze(1))          # → (B, F)
            
                features = features.view(x_batch.size(0), -1)  # flatten
            
                
                theta_hat = linear_subnets[chr_id-1](features).squeeze()  # → (B,)
                
            # train_losses = loss(theta_batch,x_batch, DNN)
                train_losses = loss(theta_batch,theta_hat)
            ...
            train_loss_chr_average += train_loss_average
            ...
            with torch.no_grad():
                for batch in val_loader['chr'+str(chr_id)]:
                    ...
                    val_losses = loss(theta_batch,theta_hat)
        ...
        val_loss_list.append(val_loss_chr_average/dim)
    ...
    plt.legend()
    #plt.show()
    plt.savefig(path+f"loss_layers_kernel_{kernel_size}_conv_{out_channels_per_layer[0]}_{out_channels_per_layer[1]}_theta_16.png")
    ...
    torch.save({'cnn': cnn_subnet.state_dict(),'mlps': [mlp.state_dict() for mlp in linear_subnets]}, "dnn.pth")
...
#                     if sum_stat:
#                         DNN = torch.load(path+f"chr_{chr_id}_dnn.pkl", map_location=torch.device('cpu'))
#                         #print(DNN)
#                         C_ref_row = torch.from_numpy(C_ref_row).float() 
...
#     for nb_seq in range(10,nb_seq+1):
