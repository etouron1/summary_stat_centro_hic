...
def main(ctx: mlxp.Context)->None:
    ...
    path = f"simulation_little_genome/{dim_ref}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/transformer/row/per_chr/3_pos_per_bloc/cls_token/"
    ...
    if 1:
        ...
        print(DNN)
        # with open(path+f"chr{ctx.config['chr_id']}_dnn_structure.txt", "w") as f:
        #  f.write(str(DNN))
        ...
        taille = ['chr3', 'chr6', 'chr9', 'chr12', 'chr15']
        croissant_theta_ref = {key : [] for key in taille}
        croissant_theta_simu = {key : [] for key in taille}
  
        print('chr'+str(ctx.config['chr_id']))
        ...
        while epoch <= max_num_epochs: 
            ...
            estimation_train = 0
            for batch in train_loader['chr'+str(ctx.config['chr_id'])]:
                optimizer.zero_grad()
                ...
                theta_hat = DNN(x_batch, nb_bead_ref)*prior_range_ref[ctx.config['chr_id']-1]  
                
                estimation_train += torch.mean(torch.abs(theta_hat.squeeze(-1)-theta_batch*prior_range_ref[ctx.config['chr_id']-1]))
            ...
            nb_simu_train = len(train_loader['chr'+str(ctx.config['chr_id'])])

            distance_to_theta_train['chr'+str(ctx.config['chr_id'])].append(estimation_train.item()*1.0/nb_simu_train)
            ...
            with torch.no_grad():
                ...
                for batch in val_loader['chr'+str(ctx.config['chr_id'])]:
                    ...
                    theta_hat = DNN(x_batch, nb_bead_ref)*prior_range_ref[ctx.config['chr_id']-1]  
                    estimation_val += torch.mean(torch.abs(theta_hat.squeeze(-1)-theta_batch*prior_range_ref[ctx.config['chr_id']-1]))

                    val_losses = loss(theta_batch,x_batch, DNN)
                    val_loss_sum += val_losses.sum().item()

            nb_simu_val = len(val_loader['chr'+str(ctx.config['chr_id'])])

            distance_to_theta_val['chr'+str(ctx.config['chr_id'])].append(estimation_val.item()*1.0/nb_simu_val)
            
           
            
            for num_chr in range(ctx.config['chr_id'], ctx.config['chr_id']+1):
                ...
                theta_hat = DNN(C_ref_row, nb_bead_ref).item()*prior_range_ref[num_chr-1]
                loss_theta_ref.append(((theta_hat-theta_ref)/prior_range_ref[num_chr-1])**2)
                ...
                for k in range(10):
                    ...
                    # plt.show()
                    theta_hat = DNN(C_tmp_row, nb_bead_ref).item()*prior_range_ref[num_chr-1]
                    
                    distance_simu+=torch.abs(theta_hat-theta[num_chr-1])*0.1
                
                distance_simu_to_theta['chr'+str(num_chr)].append(distance_simu)
            ################ test C croissant ######################
            chr_id = 'chr'+str(ctx.config['chr_id'])
            for taille in croissant_theta_ref.keys():
                ...
                C_ref_row = C_ref_16_chr[start_bead_croissant[chr_id]:start_bead_croissant[chr_id] + nb_bead_croissant[chr_id], :start_bead_croissant[taille] + nb_bead_croissant[taille]]
                C_ref_row = C_ref_row.reshape(1,1,C_ref_row.size(0), C_ref_row.size(1))
                C_ref_row = pad_input_row(patch_size, C_ref_row, ctx.config['chr_id'], chr_seq_croissant, start_bead_croissant, nb_bead_croissant)
                # print(taille)
                # print(C_ref_row.size())
                # plt.matshow(C_ref_row[0,0,:,:])
                # plt.show()
                theta_ref = chr_cen_ref[chr_id]
                theta_hat = DNN(C_ref_row, nb_bead_croissant).item()*prior_range_ref[num_chr-1]
                croissant_theta_ref[taille].append(torch.abs(theta_hat-theta_ref))
                ...
                croissant_simu = 0
                for k in range(10):
                    theta = prior_croissant.sample()

                    C_tmp = simulator_croissant(theta, chr_seq_croissant, start_bead_croissant, nb_bead_croissant, nb_tot_bead_croissant, resolution, sigma_spot, noisy_ref)  
                    C_tmp = C_tmp.reshape(1,1,C_tmp.size(0), C_tmp.size(1))
                    
                    C_tmp_row = C_tmp[:,:, start_bead_croissant[chr_id]:start_bead_croissant[chr_id] + nb_bead_croissant[chr_id], :start_bead_croissant[taille] + nb_bead_croissant[taille]]
                    C_tmp_row = pad_input_row(patch_size, C_tmp_row, ctx.config['chr_id'], chr_seq_croissant, start_bead_croissant, nb_bead_croissant)
                    # print(C_tmp_row.size())
                    # plt.matshow(C_tmp_row[0,0,:,:])
                    # plt.show()
                    theta_hat = DNN(C_tmp_row, nb_bead_croissant).item()*prior_range_ref[num_chr-1]
                    croissant_simu += torch.abs(theta_hat-theta[ctx.config['chr_id']-1])*0.1
                croissant_theta_simu[taille].append(croissant_simu)
            ...
            # )
            # train_loss_average = train_loss_sum / (
            #         len(train_loader)*len(train_loader['chr'+str(ctx.config['chr_id'])]) * train_loader['chr'+str(ctx.config['chr_id'])].batch_size  # type: ignore
            #     ) #average loss over all items of all batchs of all chr
            train_loss_average = train_loss_sum / (
                    len(train_loader['chr'+str(ctx.config['chr_id'])]) * train_loader['chr'+str(ctx.config['chr_id'])].batch_size  # type: ignore
                ) #average loss over all items of all batchs of all chr
            # Take mean over all validation samples.
            # val_loss = val_loss_sum / (
            #     len(val_loader) * len(val_loader['chr'+str(ctx.config['chr_id'])]) * val_loader['chr'+str(ctx.config['chr_id'])].batch_size  # type: ignore
            # )
            val_loss = val_loss_sum / (
                len(val_loader['chr'+str(ctx.config['chr_id'])]) * val_loader['chr'+str(ctx.config['chr_id'])].batch_size  # type: ignore
            )
        ...
        plt.tight_layout()
        plt.show()
        # plt.savefig(path+f"chr_{ctx.config['chr_id']}_{nb_train_dnn}_loss_c_ref_patch_size_{patch_size}_embed_dim_{embed_dim}_nb_blocks_{depth}_nb_heads_{num_heads}.png")
        ############### croissant test #################
        for num_chr in range(ctx.config['chr_id'], ctx.config['chr_id']+1):
            # print('chr'+str(num_chr))
            plt.figure()
            for taille in croissant_theta_ref.keys():
                plt.plot(range(epoch), croissant_theta_ref[taille], label=taille)
            plt.title('chr'+str(ctx.config['chr_id']) + ' ref - croissant' + f'depth_{depth}_nb_head_{num_heads}')
        ...
        for num_chr in range(ctx.config['chr_id'], ctx.config['chr_id']+1):
            # print('chr'+str(num_chr))
            plt.figure()
            for taille in croissant_theta_simu.keys():
                plt.plot(range(epoch), croissant_theta_simu[taille], label=taille)
            plt.title('chr'+str(ctx.config['chr_id']) + ' simu - croissant' + f'depth_{depth}_nb_head_{num_heads}')
        ...
        for num_chr in range(ctx.config['chr_id'], ctx.config['chr_id']+1):
            ...
            plt.title('chr'+str(ctx.config['chr_id']) + ' ref' + f'depth_{depth}_nb_head_{num_heads}')
            plt.ylabel('|theta_ref - DNN(C_ref)|')
            plt.axhline(y=resolution)
            plt.tight_layout()
            plt.show()
        ...
        for num_chr in range(ctx.config['chr_id'], ctx.config['chr_id']+1):
            ...
            plt.title('chr'+str(num_chr) + ' train/val' + f'depth_{depth}_nb_head_{num_heads}')
            plt.ylabel('|theta - DNN(C)|')
            plt.axhline(y=resolution)
            plt.legend()
            plt.tight_layout()
