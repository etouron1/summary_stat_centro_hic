...
path_sbi = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/row/sbi/"
...
def get_simulations_sbi_pretrained(chr_id, proposal, round, nb_train, DNN):
    ...
    for k in range(nb_train):
        ...
        C_tmp = simulator_row(chr_id, theta_simu, resolution, sigma_spot, noisy)
        ...
        #print(DNN(C[k]).size())
        #C[k] = C_tmp
        theta_reg[k] = DNN(C_tmp.unsqueeze(0)).squeeze(0)[chr_id-1]
...
if 0:
    ...
    path_dnn = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/row/summary_stat/16_theta/"
    nb_seq = 10
    inference_method = NPE
    density_estimator = "nsf"
    for chr_id in range(3,5):
        prior = utils.BoxUniform(torch.ones(1), prior_range[chr_id-1]-1)
        proposal = prior
    
        DNN = torch.load(path_dnn+f"chr_{chr_id}_dnn.pkl", map_location=torch.device('cpu'))
        ...
        for k in range(nb_seq+1):
            ...
            theta, theta_reg = get_simulations_sbi_pretrained(chr_id, proposal, k, nb_train, DNN)
            ...
            if origin == "true":
                C_ref =  torch.from_numpy(np.load(f"ref/{dim}_chr_{chr_id}_ref_{resolution}_norm_HiC_duan_intra_all.npy")).float()
                if wavelets=="wavelets":
                    C_ref, (LH, HL, HH) = pywt.dwt2(C_ref, 'bior1.3')
                    C_ref = torch.from_numpy(C_ref) 

            else:
                C_ref = simulator_row(chr_id, list(chr_cen.values()), resolution, sigma_spot, noisy)
                if wavelets=="wavelets":
                    C_ref, (LH, HL, HH) = pywt.dwt2(C_ref, 'bior1.3')
                    C_ref = torch.from_numpy(C_ref)

            proposal = posterior.set_default_x(DNN(C_ref.unsqueeze(0)).squeeze(0)[chr_id-1])
