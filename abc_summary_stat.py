...
## swap 14-16 ###
# chr_seq_3_chr_end_swap = { "chr1": 948063, "chr2": 1091290,  "chr3": 784334}
# chr_cen_3_chr_end_swap = { 'chr1': 556070, 'chr2': 326703,  'chr3': 628877}
#################
...
if dim==3:
    ...
    ### swap 14-16 ####
    # chr_seq = chr_seq_3_chr_end_swap
    # chr_cen = chr_cen_3_chr_end_swap
    # prior_range = torch.tensor([ 948063, 1091290,  784334])
    ###################
...
def simulator(theta, resolution, sigma_spot, noisy):
    ...
    return  C_simu #torch.triu(C_simu).transpose(1,0)  #torch.flip(C_simu, dims=[0, 1]).T
...
def get_simulations(nb_train_dnn):
    ...
    C = torch.zeros(nb_train_dnn,C_ref.size(0), C_ref.size(1))
...
if origin=="true":
    ...
    ### swap 14-16 ###
    # C_ref = torch.flip(C_ref, dims=[0, 1]).T
    ##################

    # C_ref = C_ref + torch.triu(C_ref).transpose(1,0)
    # C_ref = torch.triu(C_ref).transpose(1,0)
...
if 1:
    ...
    # DNN0 = torch.nn.ReLU()
    # DNN1 =torch.nn.Conv2d(1, 3, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2))
    # DNN2 =torch.nn.MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1, ceil_mode=False)
    # DNN3 = torch.nn.Conv2d(3, 6, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2))
    ...
    DNN = CNNEmbedding(input_shape=input_dim, in_channels=1, output_dim=output_dim, num_conv_layers=num_conv_layers, num_linear_layers=nb_hidden_layers, num_linear_units=first_hidden_dim, kernel_size=kernel_size, pool_kernel_size=pool_kernel_size, out_channels_per_layer=out_channels_per_layer)
    ...
    while epoch <= 200:
        ...
        for batch in train_loader:
            ...
            # # vmin = min(C_ref.min(), x_batch[0].min())
            # # vmax = max(C_ref.max(), x_batch[0].max())
            # vmin = 0
            # vmax = 1
            
            # ax[0].matshow(C_ref/C_ref.max(),vmin=vmin, vmax=vmax, cmap='viridis')
            # ax[1].matshow(x_batch[0]/x_batch[0].max(),vmin=vmin, vmax=vmax, cmap='viridis')
            # plt.show()
        ...
        # | theta_ref - DNN(C_ref) |
        theta_hat = (DNN(C_ref.unsqueeze(0))*prior_range).squeeze(0)
        ...
        # loss with C_ref
        loss_theta_ref.append(torch.mean((DNN(C_ref.unsqueeze(0))-theta_ref/prior_range)**2, dim=1).item())
        ...
        for k in range(10):
            ...
            # ax[0].matshow(C_ref/C_ref.max(),vmin=vmin, vmax=vmax, cmap='viridis')
            # ax[1].matshow(C_tmp/C_tmp.max(),vmin=vmin, vmax=vmax, cmap='viridis')
            # plt.show()

            theta_hat = (DNN(C_tmp.unsqueeze(0))*prior_range).squeeze(0)
