...
def main(ctx: mlxp.Context)->None:
    ...
    if dim==3:
        chr_seq = {k: chr_seq_16_chr[k] for k in chr_to_keep}
        chr_cen = {k: chr_cen_16_chr[k] for k in chr_to_keep}
        prior_range = torch.tensor(list(chr_seq.values()))
        print(chr_seq, chr_cen, prior_range)
    ...
    def get_simulations_C(nb_train):
        ...
        C = torch.zeros(nb_train, C_ref.size(0), C_ref.size(1), device=device)
        # print(C.size())
    ...
    if 1:
        ...
        while epoch <= max_num_epochs:
            ...
            for k in range(sum(len(loader) for loader in train_loader.values())):
                ...
                estimation_train += torch.mean(torch.abs((theta_hat-theta_batch)*prior_range[chr_id-chr_start]), dim=0).item()
            ...
            with torch.no_grad():
                ...
                for chr in range (train_chr_start, train_chr_end+1):
                    ...
                    for batch in val_loader['chr'+str(chr)]:
                        ...
                        estimation_val += torch.mean(torch.abs((theta_hat-theta_batch)*prior_range[chr-chr_start]), dim=0).item()
            ...
            for num_chr in range(train_chr_start, train_chr_end+1):
                ...
                theta_hat = mlp_subnet(features).squeeze().item()*prior_range[num_chr-chr_start]  # (B, 1) 
                loss_ref_average += (theta_hat/prior_range[num_chr-chr_start]-theta_ref/prior_range[num_chr-chr_start])**2
                ...
                for k in range(nb_C_synth):
                    ...
                    theta_hat = mlp_subnet(features).squeeze().item() * prior_range[num_chr-chr_start]  # (B, 16) 
                    
                    distance_simu+=torch.abs(theta_hat-theta[num_chr-chr_start]).item()*0.1
        ...
        min_simu = min(values)
        max_simu = max(values)
        moy_simu = sum(values)/len(values)

        values = distance_to_theta['chr'+str(ctx.config['chr_train'])][-50:]
        min_ref = min(values)
        max_ref = max(values)
        moy_ref = sum(values)/len(values)
