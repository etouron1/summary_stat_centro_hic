...
from fully_connected import FCEmbedding
...
class CNNEmbedding(nn.Module):
    def __init__(
        ...
        for ii in range(num_conv_layers):
            ...
            pool = pool_module(kernel_size=pool_kernel_size[ii])
        ...
        self.linear_subnet = FCEmbedding(
            ...
            * torch.prod(torch.tensor(cnn_output_size)),
            # input_dim=int(out_channels_per_layer[-1]
            # * cnn_output_size[0]*0.5*(cnn_output_size[1]+1)),
            
            output_dim=output_dim,
            num_layers=num_linear_layers,
            num_hiddens=num_linear_units,
        )
        # print(self.cnn_subnet)
        # print(self.linear_subnet)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # triu_indices = torch.triu_indices(x.size(2), x.size(3))
        # y = torch.tensor([])
        # for a in range(x.size(1)):
        #     y=torch.hstack((y, x[0][a][triu_indices[0], triu_indices[1]]))
        # x = y.unsqueeze(0)
        
        # import matplotlib.pyplot as plt
        # for a in range(6):
        #     fig, ax = plt.subplots(1,2)
        #     ax[0].matshow(torch.triu(x[0][a]).detach().numpy())
            
        #     ax[1].matshow(x[0][a].detach().numpy())
        #     plt.show()
        # flatten for linear layers.
        # x = vectorise_upper_matrix(self.chr_seq, x, self.resolution)
