...
from transformer_centromere import BioVisionTransformer
...
path = f"simulation_little_genome/{dim}_chr/{origin}/res_{resolution}/noisy/sigma_{sigma_spot}/transformer/sin_cos_4_levels_pos/mlp/"

def pad_input(patch_size, C):
...
def get_simulations(nb_train_dnn, patch_size):

    theta = prior.sample((nb_train_dnn,))
    sigma_spot = "variable"
    noisy = 1
    # C = torch.zeros(nb_train_dnn,C_ref.size(0))
    C = torch.zeros(nb_train_dnn,1,C_ref.size(2), C_ref.size(3))
    # print(C.size())
    ...
    for k in range(nb_train_dnn):
        ...
        C_tmp = C_tmp + torch.triu(C_tmp).transpose(1,0)
        # print(C_tmp.size())
        C_tmp = pad_input(patch_size, C_tmp)
...
def get_dataloaders(patch_size,
        ...
        theta, C  = get_simulations(nb_train_dnn, patch_size)
...
if 1:
    ...
    DNN = BioVisionTransformer(img_size=C_ref.size(2), in_chans = 1, patch_size=patch_size, embed_dim = embed_dim,no_embed_class=True, device=device, depth = depth, num_heads=num_heads, chr_seq=chr_seq, chr_cen=chr_cen, dim=dim, resolution=resolution)
    print(DNN)
    ...
    while epoch <= max_num_epochs: 
        ...
        plt.matshow(C_ref[0,0,:,:])
        plt.show()
        
        # | theta_ref - DNN(C_ref) |
        theta_hat = (DNN(C_ref)*prior_range).squeeze(0)
        
        for chr_id in range(1,dim+1):
            distance_to_theta['chr'+str(chr_id)].append(torch.abs(theta_hat-theta_ref)[chr_id-1].item())
            
        # loss with C_ref
        loss_theta_ref.append(torch.mean((DNN(C_ref)-theta_ref/prior_range)**2, dim=1).item())
    ...
    for num_chr in range(1,dim+1):
        ...
        plt.tight_layout()
        plt.show()
        # plt.savefig(path+f"{nb_train_dnn}_distance_to_theta_chr_{num_chr}.png")
    ...
    for num_chr in range(1,dim+1):
        ...
        plt.tight_layout()
        plt.show()
        # plt.savefig(path+f"{nb_train_dnn}_distance_simu_to_theta_chr_{num_chr}.png")
    ...
    print(f"theta_ref : {theta_ref}, DNN(C_ref) : {DNN(C_ref)*prior_range}\n")
    print(f"||theta_ref - DNN(C_ref)|| = {torch.sqrt(torch.mean((DNN(C_ref)*prior_range-theta_ref)**2))}")
